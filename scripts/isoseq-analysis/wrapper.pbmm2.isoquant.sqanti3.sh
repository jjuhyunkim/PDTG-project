...
echo -e "sh ~/code/_submit_norm.sh 50 60G isoquant_${output_prefix} ${pipeline}/isoquant.sh \
        "${ref} ${gtf} ${output_prefix}.pbmm2.bam ${output_prefix}" \
        --dependency=afterok:`sed '2!d' step1.pbmm2.sbatch` > step2.isoquant.sbatch"

sh ~/code/_submit_norm.sh 50 60g isoquant_${output_prefix} ${pipeline}/isoquant.sh \
	"${ref} ${gtf} ${output_prefix}.pbmm2.bam ${output_prefix}" \
	--dependency=afterok:`sed '2!d' step1.pbmm2.sbatch` > step2.isoquant.sbatch
conda deactivate
fi
